...
HWND MakeWindow(const wchar_t* windowClassName, HINSTANCE hInst, const wchar_t* windowTitle, uint32_t width, uint32_t height)
{
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    HWND hWnd = MakeWindow(windowClassName, hInstance, L"Learning DirectX 12", 1280, 720);
    RECT windowRect;
    GetWindowRect(hWnd, &windowRect);

    ComPtr<ID3D12Device2> device = CreateDevice(GetAdapter(false));

    app = new Application();
    app->initialize(hWnd, device, tearingSupported);
